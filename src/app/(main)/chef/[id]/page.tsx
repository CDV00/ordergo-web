...
import { Phone, Mail, Briefcase, UtensilsCrossed, CheckCircle } from "lucide-react";
...
export default function ChefDetailPage() {
  ...
  const handleMarkItemAsReady = (orderId: string, itemName: string) => {
    alert(`Đầu bếp ${chef?.name} đã hoàn thành món "${itemName}" của đơn hàng ${orderId}.`);
    ...
    return (
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col items-center justify-center p-4 lg:pl-50">
          <h1 className="text-2xl font-bold text-red-500">Lỗi: Không tìm thấy đầu bếp</h1>
      ...
      <div className="mt-16 flex flex-1 flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8 lg:pl-50">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Mã đầu bếp: {chef.id}</p>
          </div>
          <div className="flex items-center space-x-2">
            <Button variant="outline" className="min-h-[44px] min-w-[44px] bg-transparent" asChild>
              <Link href="/chef">Quay lại màn hình Bếp</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Card className="md:col-span-1">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <Badge className={getStaffStatusColor(chef.status)}>{chef.status}</Badge>
              </div>
              <div className="flex items-center justify-between">
          ...
          <Card className="md:col-span-1 lg:col-span-2">
            <CardHeader className="pb-3">
              ...
              <CardDescription>Danh sách các món ăn đang chờ được chế biến</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
                ...
                <ul className="space-y-3">
                  {cookingItems.map((item, index) => (
                    <li
                      ...
                      className="flex flex-col items-start justify-between border-b pb-2 last:border-b-0 last:pb-0 sm:flex-row sm:items-center"
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleMarkItemAsReady(item.orderId, item.name)}
